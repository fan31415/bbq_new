...
gtk_window_set_icon(GTK_WINDOW (soft_set),create_pixbuf("pic/logo.bmp"));
	...
	for (i = 0; i < 7; i++) {
		...
		case 0:	
			...
			g_signal_connect(GTK_OBJECT(entry),"activate",G_CALLBACK(entry_name),(gpointer)entry);

			label = gtk_label_new ("组名：");
			gtk_table_attach_defaults(GTK_TABLE(table),label,0,1,1,2);
			entry = gtk_entry_new();
			gtk_table_attach_defaults(GTK_TABLE (table), entry, 1,4,1,2);
			g_signal_connect(GTK_OBJECT(entry),"activate",G_CALLBACK(entry_group),(gpointer)entry);
			
			label = gtk_label_new ("个性签名：");
			gtk_table_attach_defaults(GTK_TABLE(table),label,0,1,2,3);
			entry = gtk_entry_new();
			gtk_table_attach_defaults(GTK_TABLE (table), entry, 1,4,2,3);
			g_signal_connect(GTK_OBJECT(entry),"activate",G_CALLBACK(entry_sig),(gpointer)entry);
