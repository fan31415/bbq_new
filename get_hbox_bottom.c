...
//小Q提醒
	tip= pic_button("Icon/tips.svg");
	gtk_button_set_focus_on_click(GTK_BUTTON(tip), FALSE);
 	gtk_button_set_relief(GTK_BUTTON(tip),GTK_RELIEF_NONE);
	g_signal_connect(G_OBJECT(tip), "clicked",G_CALLBACK (create_tips),NULL);
	gtk_widget_set_tooltip_text(tip,"小Q提醒");
    	gtk_box_pack_start(GTK_BOX (hbox_bottom),tip,FALSE,TRUE,padding);
//更换皮肤
/*
...
//刷新
	...
	g_signal_connect(G_OBJECT(refresh), "clicked",G_CALLBACK (send_info),NULL);
